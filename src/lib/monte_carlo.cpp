...
/// @param m 分子模型
/// @param out 输出容器，存储多个优化结果（按能量排序）
/// @param p 原子间相互作用预计算对象
/// @param ig 插值网格对象
/// @param corner1 搜索空间的第一个角点
/// @param corner2 搜索空间的第二个角点
/// @param increment_me 可增量对象指针，用于进度跟踪
/// @param generator 随机数生成器
/// @note 算法流程：随机初始化→蒙特卡罗采样→局部优化→保存最优结果
